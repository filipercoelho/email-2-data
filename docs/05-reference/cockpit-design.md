...
| --- | --- | --- | --- | --- | --- |
| **D1** | **Response clock + thread state** | per-thread *who-owes-whom* + age + risk score + sort key. **ADR-036**: the group-naming **obligation** now folds from the last decisive `speech_act × direction × purpose` (`cockpit.derive_obligation`) — the clock only colours+sorts; `_legacy_obligation` is the pre-`triage --full` fallback. **ADR-051**: an `outbound` message after the decisive one discharges an owed REPLY (`OWE_REPLY`→`AWAIT_THEM`), never an owed PAYMENT; the clock also emits `gap_hours`/`anchored_at_last` so the timeline's debt chip only speaks as debt over the segment it actually spans | I1 · P4 | `crm.interactions` (thread_root, direction, date, `speech_act`), `AWAITED_OUTBOUND`/`CLOSING_PURPOSES` | **M** |
