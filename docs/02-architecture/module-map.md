...
IMAP (read-only)                                  ADR-002
   ...
   │                  folders LIST-discovered every run (junk only excluded)  ADR-049
   ▼
corpus/*.eml
   │  headers.py      header bytes → text (RFC 2047 + raw 8-bit)          ADR-043
   │  envelope.py     raw MIME/charset → normalized fields
   │  attachments.py  parts → banded, hash-deduped file list        ADR-046/-048
   ▼
normalized message
...
out/crm.db
   ...
   │  locate.py       the LOCATE pass: the sentence justifying each extracted value  ADR-054 P4
   │  narrate.py      «Evolução da conversa» per multi-message thread                ADR-054 P5
...
| --- | --- | --- |
| `fetch.py` | I/O | read-only IMAP → `corpus/*.eml`; per-mailbox UID watermark. Folders come from the **server** (`LIST`) on every run, unioned with the pinned `mailboxes` — junk excluded, Trash deliberately not; a failed `LIST` falls back to the pinned list rather than narrowing to nothing — ADR-049 |
...
| `envelope.py` | parse | raw `.eml` → normalized fields (robust MIME/charset). **Body cleaning has two entry points and the split is load-bearing:** `clean_email_body(text) -> str` is **LLM-facing and frozen** — `webapp._thread_excerpts` feeds it to `clientdraft.polish_draft`, whose prompt licenses the model to restate HISTÓRICO *to a client*, so signature lines there would leak counterparty staff names/roles/NIFs into outgoing drafts. `clean_email_body_parts(text) -> (body, signature)` returns the closing block instead of destroying it, and **only `/api/thread` opts in** (`body_sig`, rendered collapsed in `.tsig`). A caller cannot opt in by forgetting to — fila-evidence §Phase 2 |
| `attachments.py` | present | a thread's parts → one **deduped, banded** file list: FICHEIROS / IMAGENS / ASSINATURAS, each an INFERENCE carrying its own `band_evidence`. Dedup is sha256 of the decoded bytes, never the filename. Walks parts under the exact predicate `attachment_part` re-walks, so banding is additive and no 📎 link is repointed — ADR-046. **ADR-048**: art we send that recurs across ≥`BRANDING_MIN_THREADS` distinct threads is *omitted* from the funnel (spread comes from `crm.db.asset_spread`); the drop happens in `fold_thread`, never in `message_parts`, or the positional 📎 links break |
| `signals.py` | Tier-0 | header facts: direction, bulk/automated, looks-forwarded |
| `extract.py` | Tier-0 | deterministic values: nif/iban authoritative + amount/date/doc candidates. **Its outputs are folded** (`_fold`: NFKD → strip combining marks → casefold) and therefore **not substrings of the body** — never compute a display offset from them. `cockpit_ui`'s `_EV_AMOUNT`/`_EV_NIF`/`_EV_IBAN` mirror these patterns client-side for the evidence highlight and must be kept in step — fila-evidence §Phase 3 |
...
| `crm.py` | rollup | interactions (event log) + contacts (person rollup) from headers+verdicts; also measures the ADR-048 `asset_spread` branding register in the same corpus walk |
...
| `locate.py` | evidence | the **locate pass** (ADR-054 Phase 4): a separate LLM call that returns the verbatim sentence justifying each already-extracted value, validated against the real body before anything is stored (echo → whitespace-tolerant match → uniqueness → a **per-field** value-in-quote gate) and written to `out/evidence.jsonl`. The stored quote is the **email's own text at the matched span**, never the string the model typed. Its `find_spans` is mirrored by `cockpit_ui.evLocateQuote` client-side and the two are pinned to agree by an executing test |
| `narrate.py` | evidence | the **narrative pass** (ADR-054 Phase 5): one call per thread with ≥ 2 messages → `out/narratives.jsonl`. Messages are presented as ordinals `m1..mN`, so a beat citing anything else is discarded before storage; the **date is attached server-side** from the real row. The incremental gate is a content **watermark** (sha256 over each message's id/date/direction/purpose/speech_act/counterparty/entities/subject), so a re-triage re-narrates even when no message arrived |
...
| `cockpit_ui.py` | UI kit | the shared shell every lens renders through: nav + identity control (`page(person=…)`, `person=None` is default-deny), the single `fetchJSON` network seam, `forbidden_page()`, `account_page()` — ADR-034/-040/-041. Also the shared message kit: `msgHTML` and its **three** collapsible regions (`.tsig` «assinatura» · `.tquote` «mensagem citada» · `.rawbody` «ver original»), all wired by `msgWireQuoteToggles` via `nextElementSibling` — **never insert an element between a toggle and its target**. The evidence highlight (`evMatches`/`evLocate`/`evHighlight`) lives here too, painted with the CSS Custom Highlight API over live `Range`s so it mutates no DOM — fila-evidence §Phase 2/§Phase 3 |
...
The queue lives at **`/fila`** and is otherwise unchanged. The Fila's **own** deep links survive
because `syncURL()` rebuilds the address from `location.pathname` — but that argument covers only
links the Fila writes for *itself*, and taking it for the whole app is what left six **cross-page**
links pointing at `/?thread=` (and `/?search=`, `/?domain=`, `/?focus=`), every one of them landing
on Início, which reads no query parameter. **The rule:** a link that leaves the page it is written on
must name its route in full. `/` does not redirect: a redirect would put the cockpit back on the
landing URL, which is the thing the ADR removed — and would mask the next such regression rather
than surface it.
...
open threads → the Fila (`/fila?thread=`), pending decisions → Para ti, projects → the workbench
(`/projetos/<pid>`), and a per-message timeline → **the Fila too** (`/fila?thread=`), not the legacy
`/inbox` report — sending 111 history links there dumped the user into a surface that contradicts
these pages (ADR-028). Every one of these cross-page links carries the `/fila` prefix **explicitly**:
there is no `location.pathname` to inherit the way the Fila's own `syncURL()` has, so they were the
exact links ADR-044 broke when it moved the Fila off the root. The same
...
The **Projetos workbench** (`/projetos/<pid>`) is a **seven-tab** strip — *Especificação · Origem ·
Ficheiros · Linha do tempo · Email ao cliente · Descritivo · Registar* — so the page is never one long
scroll; only the active panel renders, contradictions sit in a banner above the tabs
([ADR-015](../03-decisions/adr-015-knowledge-capture-claim-ledger.md)).
**Ficheiros** ([ADR-052](../03-decisions/adr-052-the-project-file-list-spans-every-thread-and-the-intake-channel.md))
is the project's whole file list: every thread's ADR-046 funnel **plus** its intake capture media,
merged and content-deduped, each tile naming the email (or the capture) that first carried it. It
makes no fetch of its own — `loadSource()` already walks the roots, one `try` **per root** so an
ungranted or dangling thread costs only itself and says so («⚠ N de M conversas não carregou»). The
email half is folded in the CLIENT deliberately: `_may_open_project` is ANY-thread, so a server-built
list would leak filenames from mail the reader was never granted; folding client-side forces every
root through the per-thread ADR-045 check. The capture half is
`GET /api/projects/<pid>/captures`, an ADR-046 funnel block whose items are **hashed**, so a drawing
that arrived both by mail and by Telegram is one row.
